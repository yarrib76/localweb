...
class AbmPersonal extends Controller
{
    ...
    {
        $usuarios = DB::select('SELECT users.id, name,email,codigo, rolesweb.tipo_role as rol, foto, vendedores.nombre as vendedora,
                                hora_ingreso, hora_egreso FROM samira.users
    ...
    public function guardar()
    {
        ...
        $hora_ingreso = Input::get('hora_ingreso');
        $hora_egreso = Input::get('hora_egreso');
        ...
        DB::select('UPDATE samira.users SET name = "'.$nombre.'", email = "'.$email.'", codigo = "'.$codigo.'",
                    ...
                    hora_ingreso = "'.$hora_ingreso.'", hora_egreso = "'.$hora_egreso.'"
                    where id = "'.$user_id.'"');
